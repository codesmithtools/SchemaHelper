    ...
    public class QuickStartUtils {
        ...
        private static void CopyDirectory(string pathFrom, string pathTo, int level) {
            if (level <= 5) {
                ...
                foreach (string subdir in subdirEntries) {
                    // Do not iterate through reparse points
                    if ((File.GetAttributes(subdir) & FileAttributes.ReparsePoint) != FileAttributes.ReparsePoint && (File.GetAttributes(subdir) & FileAttributes.Hidden) != FileAttributes.Hidden)
